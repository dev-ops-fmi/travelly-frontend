...
export class TripService {
  private apiUrl = `${API_PREFIX}/trips`;
  private tripId = -1;
  ...
  getRecommendedAccommodations(
    ...
    return this.http.get<Accommodation[]>(
      `${this.apiUrl}/${tripId}/accommodations/recommend?${queryParams}`
    );
  }
  ...
  getRecommendedActivities(
    ...
    return this.http.get<Activity[]>(
      `${this.apiUrl}/${tripId}/activities/recommend?${queryParams}`
    );
  }

  getRecommendedTransportationOptions(
    tripId: number,
    queryParams: string
  ): Observable<TransportationOption[]> {
    return this.http.get<TransportationOption[]>(
      `${this.apiUrl}/${tripId}/transportationOptions/recommend?${queryParams}`
    );
  }
